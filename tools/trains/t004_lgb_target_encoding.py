...
# from tools.utils.encodings import TargetEncoder
from tools.utils.encodings import target_encode
...
@dec_timer
def t004_lgb_train(args, script_name, configs, logger):
    '''
    policy
    ------------
...
#     target_encoders = []
    ...
    for i, idxes in tqdm(list(enumerate(folds))):
        ...
        fold_features_df = features_df.iloc[trn_idx]
        fold_val_features_df = features_df.iloc[val_idx]
        fold_target = target[trn_idx]
        fold_val_target = target[val_idx]
        fold_test_features_df = test_features_df.copy()
        print('target encoding ...')
        for col in tqdm(configs['te_features']):
            trn_col, val_col = target_encode(
                fold_features_df[col], fold_val_features_df[col], fold_target,
                smoothing=0.5, noise_level=0.01)
            _, tst_col = target_encode(
                fold_features_df[col], fold_test_features_df[col], fold_target,
                smoothing=0.5, noise_level=0.01)
            fold_features_df[col] = trn_col.values
            fold_val_features_df[col] = val_col.values
            fold_test_features_df[col] = tst_col.values
        encoded_test_features.append(fold_test_features_df)
        # target encoding
        # TE = TargetEncoder(logger=logger)
#        TE = TargetEncoder(
#            impute_missing=False,
#            handle_unknown='ignore',
#            cols=configs['te_features'])
#        # fit
#        TE.fit(fold_features_df[configs['te_features']], fold_target)
#        # transform
#        fold_te_features = TE.transform(
#            fold_features_df[configs['te_features']])
#        fold_val_te_features = TE.transform(
#            fold_val_features_df[configs['te_features']])
#        fold_features_df = fold_features_df.drop(
#            configs['te_features'], axis=1)
#        fold_features_df = pd.concat(
#            [fold_features_df, fold_te_features], axis=1)
#        fold_val_features_df = fold_val_features_df.drop(
#            configs['te_features'], axis=1)
#        fold_val_features_df = pd.concat(
#            [fold_val_features_df, fold_val_te_features], axis=1)
#        target_encoders.append(TE)
        ...
        train_set = lightgbm.Dataset(fold_features_df,
                                     fold_target)
        valid_set = lightgbm.Dataset(fold_val_features_df,
                                     fold_val_target)
        # train
        ...
        y_pred = booster.predict(fold_val_features_df.values,
                                 num_iteration=None)
        y_true = fold_val_target
        oofs.append(y_pred)
...
#        for booster, te in tqdm(zip(cv_model, target_encoders)):
#            fold_test_features_df = test_features_df.copy()
#            fold_test_te_features_df = te.transform(
#                fold_test_features_df[configs['te_features']])
#            fold_test_features_df = fold_test_features_df.drop(
#                configs['te_features'], axis=1)
#            fold_test_features_df = pd.concat(
#                [fold_test_features_df, fold_test_te_features_df], axis=1)
        for booster, fold_test_features_df in tqdm(zip(cv_model, encoded_test_features)):
            pred = booster.predict(fold_test_features_df.values,
                                   num_iteration=None)
            pred = pd.Series(pred)
