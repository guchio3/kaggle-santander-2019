...
def f027_f014_3nonuniq(df):
    ...
    for col in tqdm(df.columns):
        ...
        res_df['non_3uniq_real_' + col] = df[col].apply(
            lambda x: x if uniq_cnt_dict[x] > 3 else np.nan).values
    # return as id is set to the index
    res_df = res_df.set_index('ID_code').add_prefix('f027_')
    return res_df
