...
def f029_f014_4nonuniq(df):
    ...
    for col in tqdm(df.columns):
        ...
        res_df['non_4uniq_real_' + col] = df[col].apply(
            lambda x: x if uniq_cnt_dict[x] > 4 else np.nan).values
    # return as id is set to the index
    res_df = res_df.set_index('ID_code').add_prefix('f029_')
    return res_df
